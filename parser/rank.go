...
// Rank counts the number of words that match a given Noun
...
func (r *Ranking) AddRanking(n Noun, rank int) {
	switch {
	case rank > r.Rank:
		r.Rank, r.Nouns = rank, []Noun{n}
	case rank == r.Rank:
		r.Nouns = append(r.Nouns, n)
	}
}

func (m *RankOne) RankNoun(cs Cursor, n Noun) bool {
	if m.MatchesNoun(n) {
		var rank int
		for ; ; rank++ {
...
func (m *RankAll) RankNoun(cs Cursor, n Noun) bool {
	if m.MatchesNoun(n) {
		var rank, cnt int
		for {
			var matches bool
