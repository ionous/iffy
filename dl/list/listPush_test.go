...
func TestPush(t *testing.T) {
	...
	// 	t.Fatal(got)
	// }
	if got, e := push(fruit); e != nil {
		t.Fatal(e)
	} else if got != "1; Lemon" {
		t.Fatal(got)
	}
	if got, e := push(fruit, "Mango"); e != nil {
		t.Fatal(e)
	} else if got != "2; Lemon, Mango" {
		t.Fatal(got)
	}
	if got, e := push(fruit, "Mango", "Grape"); e != nil {
		t.Fatal(e)
...
func push(src []string, add ...string) (ret string, err error) {
	if run, vals, e := newListTime(src, nil); e != nil {
		err = e
	} else {
		num := getNum(run, &list.Push{"Source", FromTs(add)})
