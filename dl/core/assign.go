...
type Assignment interface {
	Assign(rt.Runtime, string, VariableTarget) error
}

type VariableTarget interface {
	SetVariable(name string, v interface{}) (err error)
}

type Assignments []*Assign

// Assign turns Assignment into a normal statement.
type Assign struct {
...
func (op *Assign) Execute(run rt.Runtime) (err error) {
	if assign := op.Assignment; assign == nil {
		err = rt.MissingEval("empty assignment")
	} else {
		err = assign.Assign(run, op.Name, run)
	}
	return
}
...
func (op *FromBool) Assign(run rt.Runtime, name string, out VariableTarget) (err error) {
	if val, e := rt.GetBool(run, op.Val); e != nil {
		err = e
	} else {
		err = out.SetVariable(name, val)
	}
	return
}
...
func (op *FromNum) Assign(run rt.Runtime, name string, out VariableTarget) (err error) {
...
func (op *FromText) Assign(run rt.Runtime, name string, out VariableTarget) (err error) {
...
func (op *FromNumList) Assign(run rt.Runtime, name string, out VariableTarget) (err error) {
	if vals, e := rt.GetNumList(run, op.Vals); e != nil {
		err = e
	} else {
		err = out.SetVariable(name, vals)
	}
	return
}
...
func (op *FromTextList) Assign(run rt.Runtime, name string, out VariableTarget) (err error) {
